...
const emailDOM = document.getElementById('email'); //Gaunamas HTML elementas su ID "email" (el. pašto įvesties laukas) ir priskiriamas kintamajam emailDOM.
...
const tosDOM = document.getElementById('tos'); //Gaunamas HTML elementas su ID "tos" (Terms of Service - taisyklių sutikimo checkbox'as) ir priskiriamas kintamajam tosDOM.

if (formDOM) {//Tikrinama, ar formos elementas egzistuoja (kintamasis formDOM turėjo būti apibrėžtas anksčiau).
    formDOM.addEventListener('submit', (e) => { //Prie formos pridedamas event listener, kuris klausosi "submit" įvykio (kai vartotojas pateikia formą).
...
tos - taisyklių sutikimo reikšmė*/
        ...
        fetch('/api/register', { //Siunčiama HTTP užklausa į serverio "/api/register" maršrutą.
            method: 'POST',
